<div>
    <div class="flex gap-5 p-20">
        <img src="{{ $product->image ? Storage::url($product->image) : asset('images/placeholder-image.jpg') }}" alt="product-images" style="border-radius: 15px;" class="object-cover w-[300px] h-[280px]">
        <div>
            <h2 class="p-1 font-medium text-2xl text-gray-800">{{ $product->name }}</h2>
            <h2 class="p-1 text-gray-500 text-base leading-relaxed">{{ $product->description }}</h2>
            ...
            <div class="my-3">
                @if (auth()->check())
                <button wire:click="addToKart({{ $product->id }})">
